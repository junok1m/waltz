...
export function WalkingHeatmap({walks,onOpenDay}:Props){
 ...
 const{weeks,maxDistance,weekdayStats,year,todayKey,currentWeekIndex}=useMemo(()=>{
   ...
   const lastWeek=startOfWeek(yearEnd);
   const currentWeekIndex=Math.floor((startOfWeek(today).getTime()-firstWeek.getTime())/604800000);
   const stats=new Map<string,DayStat>();
   for(const w of walks){const d=startOfDay(new Date(w.ended_at));if(d<yearStart||d>today)continue;const key=dayKey(d);const current=stats.get(key)??{key,date:d,distance:0,seconds:0,count:0};current.distance+=w.distance_km;current.seconds+=w.duration_seconds;current.count+=1;stats.set(key,current);}
   ...
   for(let wi=0;wi<weekCount;wi++){
     ...
     for(let di=0;di<7;di++){
       ...
       if(d<yearStart||d>yearEnd){week.push(null);continue;}
       const key=dayKey(d);const stat=stats.get(key)??{key,date:d,distance:0,seconds:0,count:0};
       if(d<=today)maxDistance=Math.max(maxDistance,stat.distance);
       week.push(stat);
     }
     weeks.push(week);
   }
   const weekdayStats=Array.from({length:7},(_,i)=>{const matching=[...stats.values()].filter(s=>((s.date.getDay()+6)%7)===i);const activeDays=matching.filter(s=>s.count>0);return{label:["Mon","Tue","Wed","Thu","Fri","Sat","Sun"][i],avg:activeDays.length?activeDays.reduce((sum,s)=>sum+s.distance,0)/activeDays.length:0,total:matching.reduce((sum,s)=>sum+s.distance,0)};});
   return{weeks,maxDistance,weekdayStats,year,todayKey,currentWeekIndex};
 },[walks]);
 ...
 function shade(day:DayStat){if(day.key>todayKey)return"#F4EFE6";if(day.distance<=0)return"#EEE8DD";const ratio=maxDistance?day.distance/maxDistance:0;if(ratio<.25)return"#DDE3CF";if(ratio<.5)return"#C1CCAA";if(ratio<.75)return"#A4B184";return"#78845C";}
 function pressDay(day:DayStat){if(day.key>todayKey)return;if(selectedKey===day.key&&day.count>0&&onOpenDay){onOpenDay(day.key);return;}setSelectedKey(day.key);}
 function centerToday(width=viewportWidth){if(!width)return;const weekPitch=13;const x=Math.max(0,currentWeekIndex*weekPitch-width/2+weekPitch/2);scrollRef.current?.scrollTo({x,animated:false});}
 return <View style={s.card}>
   <View style={s.header}><View><Text style={s.eyebrow}>CONSISTENCY</Text><Text style={s.title}>{year} waltzes</Text></View><View style={s.legend}><Text style={s.legendText}>less</Text>{["#EEE8DD","#DDE3CF","#C1CCAA","#A4B184","#78845C"].map(c=><View key={c} style={[s.legendCell,{backgroundColor:c}]}/>) }<Text style={s.legendText}>more</Text></View></View>
   ...
   <View style={s.heatmapRow}><View style={s.dayLabels}>{["M","","W","","F","","S"].map((x,i)=><Text key={i} style={s.dayLabel}>{x}</Text>)}</View><ScrollView ref={scrollRef} horizontal showsHorizontalScrollIndicator={false} onLayout={e=>{const width=e.nativeEvent.layout.width;setViewportWidth(width);requestAnimationFrame(()=>centerToday(width));}} contentContainerStyle={s.weeks}>{weeks.map((week,wi)=><View key={wi} style={s.week}>{week.map((day,di)=>day?<Pressable key={day.key} onPress={()=>pressDay(day)} style={[s.cell,{backgroundColor:shade(day)},day.key===todayKey&&s.todayCell,day.key===selectedKey&&s.selectedCell]}/>:<View key={`empty-${di}`} style={[s.cell,s.outsideYearCell]}/>)}</View>)}</ScrollView></View>
