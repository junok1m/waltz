...
type Props = { walks: Walk[]; onStartWalk: () => void; onNavigate: (tab: AppTab) => void };
...
export function HomeScreen({ walks, onStartWalk, onNavigate }: Props) {
  ...
  return <View style={s.screen}>
    ...
    <View style={s.card}>
      ...
      <View style={s.calendar}>{cells.map((day,i)=><View key={i} style={s.day}><Text style={s.dayText}>{day ?? ""}</Text>{day && days.has(day) ? <Text style={s.paw}>🐾</Text> : null}</View>)}</View>
    ...
    <Pressable style={s.start} onPress={onStartWalk}><Text style={s.startText}>🐾  START WALK</Text></Pressable>
    ...
    <View style={s.nav}><Nav icon="⌂" label="Home" active onPress={() => onNavigate("home")} /><Nav icon="⌖" label="History" onPress={() => onNavigate("map")} /><Pressable style={s.pawButton} onPress={onStartWalk}><Text style={{fontSize:27}}>🐾</Text></Pressable><Nav icon="♡" label="Community" onPress={() => onNavigate("community")} /><Nav icon="♙" label="Me" onPress={() => onNavigate("me")} /></View>
  </View>;
}
