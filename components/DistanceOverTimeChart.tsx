...
export function DistanceOverTimeChart({walks,onOpenDay}:Props){
 ...
 const barWidth=group==="day"?12:group==="week"?18:28,gap=group==="day"?5:7;
 ...
 return <View style={s.card}>
   ...
   <View style={s.chartRow}><View style={s.yAxis}><Text style={s.yTop}>{metricValueText(max,metric)}</Text><Text style={s.yBottom}>0 {metricUnit(metric)}</Text></View><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={[s.bars,{gap}]}><View style={s.baseline}/>{buckets.map((b,i)=>{const value=metricValue(b,metric),height=value<=0?2:Math.max(5,(value/max)*122),selectedBar=b.key===selectedKey;const showLabel=i===0||i===buckets.length-1||i%Math.max(1,Math.ceil(buckets.length/6))===0;return <Pressable key={b.key} onPress={()=>pressBucket(b)} style={[s.barCell,{width:barWidth}]}><View style={s.barArea}><View style={[s.bar,{height,width:barWidth,opacity:value<=0?.14:selectedBar?1:.72},selectedBar&&s.selectedBar]}/></View><Text style={s.xLabel}>{showLabel?bucketLabel(b,group):""}</Text></Pressable>})}</ScrollView></View>
