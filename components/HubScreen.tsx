...
import { Balloon, Bird, Flame, MoonStar, Umbrella } from "@sketchyicons/react-native";
...
type Props = { tab: Exclude<AppTab, "home">; walks: Walk[]; dog: Dog; onBack: () => void; onStartWalk: () => void; onSignOut: () => void };
...
export function HubScreen({ tab, walks, dog, onBack, onStartWalk, onSignOut }: Props) {
  ...
  return <View style={styles.screen}>
    <View style={styles.header}><Pressable onPress={onBack} hitSlop={12}><Text style={styles.back}>‹</Text></Pressable><Text style={styles.title}>{title}</Text><View style={styles.headerSpacer} /></View>
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      {tab === "challenges" && <>
        ...
        <Challenge icon={<Text style={styles.challengeEmoji}>🌍</Text>} title="Around the block-ish" progress={`${Math.min(totalDistance, 25).toFixed(1)}/25 km`} done={totalDistance >= 25} />
      </>}
