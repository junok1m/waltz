...
export default function App() {
  ...
  const [dogs,setDogs]=useState<Dog[]>([]), [dogsLoading,setDogsLoading]=useState(false), [walks,setWalks]=useState<Walk[]>([]), [badges,setBadges]=useState<DogBadge[]>([]);
  ...
  useEffect(()=>{if(!session?.user.id){setDogs([]);setWalks([]);setBadges([]);setTab("home");return}loadDogs(session.user.id);loadWalks()},[session?.user.id]);
  useEffect(()=>{if(dogs[0]?.id)loadBadges(dogs[0].id)},[dogs[0]?.id]);
  ...
  async function loadWalks(){try{const next=await fetchWalks();setWalks(next);return next}catch(error){console.error("Load walks error:",error);return []}}
  async function loadBadges(dogId:string){try{setBadges(await fetchDogBadges(dogId))}catch(error){console.error("Load badges error:",error)}}
